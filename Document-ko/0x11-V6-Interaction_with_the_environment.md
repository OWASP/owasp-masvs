...
이 그룹의 컨트롤은 앱이 플랫폼 API와 표준 구성요소를 안전한 방식으로 사용한다는 것을 보장합니다. 또한, 컨트롤에는 앱 간 통신(IPC)이 포함됩니다.
...
| **6.7** | MSTG‑PLATFORM‑7 | 앱의 네이티브 메소드가 웹뷰에 노출되는 경우 웹뷰가 앱 패키지에 포함된 자바스크립트만 렌더링하여야 한다. | ✓ | ✓ |
| **6.8** | MSTG‑PLATFORM‑8 | 객체 직렬화 해제는 안전한 직렬화 API를 사용하여 구현하여야 한다. | ✓ | ✓ |
| **6.9** | MSTG‑PLATFORM‑9 | 애플리케이션은 화면 오버레이 공격으로부터 자신을 보호하여야 한다. (Android 만 해당) |  | ✓ |
| **6.10** | MSTG‑PLATFORM‑10 | WebView를 삭제하기 전에 WebView의 캐시, 스토리지 및 로드된 리소스(JavaScript 등)를 지워야 한다. |  | ✓ |
